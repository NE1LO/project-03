import{h as v,b as T,a as h,i,c as E,o as I,n as q}from"./assets/quote-662fa2df.js";import"./assets/vendor-0a7943b3.js";const P="/project-03/assets/favorite-dumbbell-325968d2.png",r=document.querySelector(".favorites-exercise-container"),g=document.querySelector(".render-btn-list-pagination"),f="favorites";let o=null;const A=getComputedStyle(document.querySelector("body")).width,C=parseInt(A)<768?8:9;let b=1;const m=8,y=async t=>{try{return(await h.get(`/exercises/${t}`)).data}catch(a){console.log(a)}},M=()=>{try{o=JSON.parse(localStorage.getItem(f))}catch(t){console.log(t)}return o},B=()=>{r.innerHTML="";const t=`<div class="favorites-exercise-not-found-container">
        <img
        ...
        </li>`,"");r.insertAdjacentHTML("beforeend",a)};async function p(){if(M(),o!==null&&C===8&&o.length>8)D(b);else if(o!==null&&o.length!==0){const t=await Promise.all(o.map(a=>y(a)));_(t),v(g)}else B()}p();const H=t=>{t.preventDefault();const a=t.target;if(t.target.closest(".icon-basket ")){const s=a.classList[1],c=o.filter(n=>n!==s);localStorage.setItem(f,JSON.stringify(c)),p()}};r.addEventListener("click",H);async function D(t){const a=Math.ceil(o.length/m);T(a,t);const e=(t-1)*m,s=e+m,c=o.slice(e,s),n=await Promise.all(c.map(u=>y(u)));_(n)}g.addEventListener("click",t=>{b=Number(t.target.innerHTML),p()});const O=document.querySelector(".favorites-exercise-container"),l=document.querySelector(".modal-trane-background"),k=()=>{l.classList.remove("modal-trane-background-active"),document.removeEventListener("click",S),document.removeEventListener("keydown",w)},S=t=>{t.target===l&&k()},w=t=>{t.key==="Escape"&&k()},R=async t=>{const e=(await h.get(`exercises/${t}`)).data,s=`<div class="modal-trane">
  ...
  </div>`;l.innerHTML=s,document.querySelector(".modal-trane-btn-close").addEventListener("click",k),document.querySelector(".modal-trane-btn-rating").addEventListener("click",I)};O.addEventListener("click",t=>{t.target.classList.contains("workout-card__link-start")&&(R(t.target.dataset.id),l.classList.add("modal-trane-background-active"),document.addEventListener("click",S),document.addEventListener("keydown",w))});const d=document.querySelector(".change");document.querySelector(".page-link")||document.querySelector(".change");document.querySelector(".collection-img");function F(){const t=document.getElementById("dynamicStyles");t?t.remove():x(),j()}function j(){d.checked?localStorage.setItem("darkThemeActivated","true"):localStorage.removeItem("darkThemeActivated")}function x(){const t=document.createElement("link");t.rel="stylesheet",t.href=".././css/layout/dark-theme.css",t.id="dynamicStyles",document.head.appendChild(t)}function N(){localStorage.getItem("darkThemeActivated")==="true"&&x()}function J(){const t=localStorage.getItem("darkThemeActivated");d.checked=t==="true"}function U(){localStorage.setItem("darkThemeActivated",d.checked.toString())}window.onload=function(){N(),J()};d.addEventListener("change",function(){U(),F()});q();
